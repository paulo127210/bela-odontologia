"""
Bela Odontologia — Flask + PostgreSQL (Supabase)
Substitui o app.py raiz após a migração para o Supabase.
Diferenças em relação à versão MySQL:
  - YEAR()/MONTH()       → DATE_TRUNC('month', x)
  - DATE_FORMAT(x,'%Y-%m')→ TO_CHAR(x,'YYYY-MM')
  - CURDATE()            → CURRENT_DATE
  - DATE_SUB(x,INTERVAL) → x - INTERVAL '…'
  - exe() não usa LAST_INSERT_ID (sem retorno necessário)
"""
import os
...
def dashboard():
    ...
    total_hoje      = q1("SELECT COUNT(*) AS n FROM consultas WHERE DATE(data_hora)=CURRENT_DATE")['n']
    total_mes       = q1(
        "SELECT COUNT(*) AS n FROM consultas "
        "WHERE DATE_TRUNC('month', data_hora) = DATE_TRUNC('month', NOW())"
    )['n']
    fat_row         = q1(
        "SELECT COALESCE(SUM(valor),0) AS s FROM pagamentos "
        "WHERE DATE_TRUNC('month', data_pagamento::TIMESTAMPTZ) = DATE_TRUNC('month', NOW())"
    )
    faturamento_mes = float(fat_row['s']) if fat_row else 0.0
    ...
    proximas = q(
        ...
        "WHERE c.data_hora >= NOW() AND c.status <> 'cancelada' "
...
def pacientes_lista():
    ...
    if busca:
        like = f'%{busca}%'
        rows = q("SELECT * FROM pacientes WHERE nome ILIKE %s OR cpf LIKE %s OR telefone LIKE %s ORDER BY nome",
                 (like, like, like))
    else:
